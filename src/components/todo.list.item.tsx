    ...
    return (
        <Alert show={show} onClose={() => handleClose()} variant={"info"} dismissible={!props.isSelected}>
            ...
            <div className="d-flex justify-content-end" style={{ marginTop: "15px" }}>
                ...
                {props.isSelected ?
                    <Button onClick={() => { dispatch({ type: ActionType.CLEAR }); props.history.push(`/`); }} variant="danger" style={{ marginRight: "8px" }}>
                        Back
                </Button> :
                    <Link className="btn btn-info" onClick={() => setRecord()} to={`/select/${todo.id}`} variant="info">
                        Select
            </Link>
                }
